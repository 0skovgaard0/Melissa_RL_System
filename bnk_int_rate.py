...
class BankData:
    def __init__(self, api_key):
        ...
        self.series_ids = ["LIOR3M", "FEDFUNDS", "GS10", "GS2", "DTB3", "T5YIE", "T10YIE", "IRSTCI01EZM156N", "IRSTCB01JPM156N"]
    ...
    def clean_and_transform_data(self, data):
        ...
        cleaned_data.to_csv('bnk_int_rate_updated_data.csv', index=False)

        return cleaned_data

if __name__ == "__main__":
    # Set API key and create a BankData instance
    api_key = "af08e5017b1a9d6a033648512f73c60c"
    bank_data = BankData(api_key)

    # Fetch raw interest rate data and save to CSV file
    interest_raw_data = bank_data.fetch_data()
    interest_raw_data.to_csv("interest_raw_data.csv", index_label='date')

    # Clean and transform the data
    interest_clean_data = bank_data.clean_and_transform_data(interest_raw_data)

    # Save the preprocessed data to a new CSV file
    interest_clean_data.to_csv('bnk_int_rate_preprocessed.csv', index=False)
    
    # Make a 2.0 version, long format to merge with the other Macroeconomic data files
    df = pd.read_csv('bnk_int_rate_preprocessed.csv')
    ...
    df_long.to_csv('bnk_int_rate_preprocessed_2_0.csv', index=False)
